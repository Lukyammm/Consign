...
// ----------------------- DASHBOARD BI -----------------------
function obterOuCriarAba(nome, cabecalhos) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  } else if (cabecalhos && cabecalhos.length) {
    var existentes = sheet.getLastColumn() > 0 ? sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0] : [];
    var precisaAtualizar = cabecalhos.some(function(h) { return existentes.indexOf(h) === -1; });
    if (precisaAtualizar) {
      sheet.clear();
      sheet.getRange(1, 1, 1, cabecalhos.length).setValues([cabecalhos]);
    }
...
function garantirEstruturaDashboard() {
  var cabLog = ['timestamp_criacao', 'timestamp_inicio', 'timestamp_conclusao', 'status', 'armario_id', 'usuario_solicitante', 'usuario_atendente', 'perfil', 'unidade', 'observacoes'];
  var cabConfig = ['chave', 'valor', 'descricao'];
  var cabSnapshots = ['data', 'periodo_inicio', 'periodo_fim', 'total', 'concluidas', 'canceladas', 'taxa_conclusao', 'taxa_cancelamento', 'sla_percentual', 'sla_meta', 'backlog', 'observacoes'];
  obterOuCriarAba('LOG', cabLog);
  obterOuCriarAba('CONFIG', cabConfig);
  obterOuCriarAba('SNAPSHOTS', cabSnapshots);
}

function carregarConfiguracoesDashboard() {
  garantirEstruturaDashboard();
  ...
  var defaults = {
    sla_minutos: 30,
    limite_backlog: 15,
    alerta_aberto_minutos: 60,
    alerta_armario_travado_minutos: 120,
    email_alertas: ''
  };

  valores.slice(1).forEach(function(linha) {
    if (!linha[0]) return;
    mapa[normalizarTextoBasico(linha[0])] = linha[1];
  });

  Object.keys(defaults).forEach(function(chave) {
    if (mapa[normalizarTextoBasico(chave)] === undefined) {
      sheet.appendRow([chave, defaults[chave], '']);
      mapa[normalizarTextoBasico(chave)] = defaults[chave];
    }
  });

  return {
    slaMinutos: Number(mapa['sla_minutos']) || defaults.sla_minutos,
    limiteBacklog: Number(mapa['limite_backlog']) || defaults.limite_backlog,
    alertaAbertoMinutos: Number(mapa['alerta_aberto_minutos']) || defaults.alerta_aberto_minutos,
    alertaArmarioTravado: Number(mapa['alerta_armario_travado_minutos']) || defaults.alerta_armario_travado_minutos,
    emailAlertas: mapa['email_alertas'] || ''
  };
}

function calcularPeriodoSelecionado(params) {
  var hoje = new Date();
  var inicio, fim;
  ...
  switch ((params && params.periodo) || '7d') {
    case 'hoje':
      inicio = new Date();
    ...
    case 'mes':
      ...
      break;
    case 'personalizado':
      inicio = params.dataInicio ? new Date(params.dataInicio) : new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate() - 6);
      fim = params.dataFim ? new Date(params.dataFim + 'T23:59:59') : fim;
      break;
    default:
      inicio = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate() - 6);
      inicio.setHours(0, 0, 0, 0);
  ...
  return {
    inicio: inicio,
    fim: fim,
    inicioTexto: Utilities.formatDate(inicio, tz, 'dd/MM/yyyy'),
    fimTexto: Utilities.formatDate(fim, tz, 'dd/MM/yyyy'),
    tz: tz
  };
}
...
function calcularDuracoes(registro) {
  var criacao = converterParaData(registro.timestamp_criacao);
  var inicio = converterParaData(registro.timestamp_inicio);
  var conclusao = converterParaData(registro.timestamp_conclusao);
  var fila = null;
  var atendimento = null;
  var total = null;
  if (criacao && inicio) {
    fila = (inicio.getTime() - criacao.getTime()) / 60000;
  }
  if (inicio && conclusao) {
    atendimento = (conclusao.getTime() - inicio.getTime()) / 60000;
  }
  if (criacao && conclusao) {
    total = (conclusao.getTime() - criacao.getTime()) / 60000;
  }
  return { fila: fila, atendimento: atendimento, total: total };
}
...
function calcularPercentil(valores, percentil) {
  ...
  var pos = (percentil / 100) * (ordenados.length - 1);
...
function agruparPorDia(logs) {
  ...
  logs.forEach(function(item) {
    var criacao = converterParaData(item.timestamp_criacao);
    if (!criacao) return;
    var chave = criacao.toISOString().split('T')[0];
    mapa[chave] = (mapa[chave] || 0) + 1;
  });
  var labels = Object.keys(mapa).sort();
  return { labels: labels, valores: labels.map(function(l) { return mapa[l]; }) };
}

function agruparStatus(logs) {
  var mapa = {};
  logs.forEach(function(item) {
    var status = normalizarTextoBasico(item.status) || 'indefinido';
...
function montarHeatmap(logs) {
  var dias = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
  var matriz = dias.map(function(dia) { return { label: dia, valores: new Array(24).fill(0) }; });
  logs.forEach(function(item) {
    var criacao = converterParaData(item.timestamp_criacao);
    if (!criacao) return;
    var dia = criacao.getDay();
    var hora = criacao.getHours();
    matriz[dia].valores[hora] = (matriz[dia].valores[hora] || 0) + 1;
  });
  return matriz;
}

function calcularResumoArmarios(logs) {
  var mapa = {};
  logs.forEach(function(item) {
    var chave = normalizarIdentificador(item.armario_id) || 'Sem identificação';
    if (!mapa[chave]) {
      mapa[chave] = {
        armario: chave,
        total: 0,
    ...
    mapa[chave].total++;
    if (normalizarTextoBasico(item.status) === 'aberto') mapa[chave].abertos++;
    if (normalizarTextoBasico(item.status) === 'cancelado') mapa[chave].cancelados++;
    var dur = calcularDuracoes(item);
    if (dur.total !== null && !isNaN(dur.total)) {
      mapa[chave].tempos.push(dur.total);
    }
    var criacao = converterParaData(item.timestamp_criacao);
    if (criacao) {
      if (!mapa[chave].ultimaMovimentacao || criacao > mapa[chave].ultimaMovimentacao) {
        mapa[chave].ultimaMovimentacao = criacao;
        mapa[chave].statusAtual = item.status;
      }
    }
  });
  ...
  return Object.keys(mapa).map(function(chave) {
    ...
    return {
      ...
      total: info.total,
      abertos: info.abertos,
      cancelados: info.cancelados,
      tempoMedio: info.tempos.length ? info.tempos.reduce(function(a, b) { return a + b; }, 0) / info.tempos.length : null,
      p95: calcularPercentil(info.tempos, 95),
      ultimaMovimentacao: info.ultimaMovimentacao ? Utilities.formatDate(info.ultimaMovimentacao, Session.getScriptTimeZone(), 'dd/MM HH:mm') : '',
      statusAtual: info.statusAtual
    };
  }).sort(function(a, b) { return b.total - a.total; });
}

function filtrarLogsDashboard(params, config) {
  garantirEstruturaDashboard();
  var periodos = calcularPeriodoSelecionado(params || {});
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('LOG');
  var dados = sheet.getDataRange().getValues();
  var cabecalho = dados[0] || [];
  var indice = obterEstruturaPlanilha(sheet);
  var registros = [];

  dados.slice(1).forEach(function(linha) {
    if (!linha || !linha.length) return;
    var registro = {
      timestamp_criacao: obterValorLinha(linha, indice, 'timestamp_criacao', null),
      timestamp_inicio: obterValorLinha(linha, indice, 'timestamp_inicio', null),
      timestamp_conclusao: obterValorLinha(linha, indice, 'timestamp_conclusao', null),
      status: obterValorLinhaFlexivel(linha, indice, ['status'], ''),
      armario_id: obterValorLinhaFlexivel(linha, indice, ['armario_id', 'numero do armario'], ''),
      usuario_solicitante: obterValorLinhaFlexivel(linha, indice, ['usuario_solicitante', 'solicitante'], ''),
      usuario_atendente: obterValorLinhaFlexivel(linha, indice, ['usuario_atendente', 'atendente'], ''),
      perfil: obterValorLinhaFlexivel(linha, indice, ['perfil'], ''),
      unidade: obterValorLinhaFlexivel(linha, indice, ['unidade', 'setor'], ''),
      observacoes: obterValorLinhaFlexivel(linha, indice, ['observacoes', 'observação', 'obs'], '')
    };

    var criacao = converterParaData(registro.timestamp_criacao);
    if (!criacao) return;
    if (criacao < periodos.inicio || criacao > periodos.fim) return;

    if (params.status && params.status.length) {
      var statusNormalizado = normalizarTextoBasico(registro.status);
      if (params.status.map(normalizarTextoBasico).indexOf(statusNormalizado) === -1) return;
    }
    if (params.unidade && normalizarTextoBasico(params.unidade) !== normalizarTextoBasico(registro.unidade)) return;
    if (params.armarioId && normalizarTextoBasico(params.armarioId) !== normalizarTextoBasico(registro.armario_id)) return;
    if (params.perfil && normalizarTextoBasico(params.perfil) !== normalizarTextoBasico(registro.perfil)) return;
    if (params.solicitante && normalizarTextoBasico(params.solicitante) !== normalizarTextoBasico(registro.usuario_solicitante)) return;
    if (params.atendente && normalizarTextoBasico(params.atendente) !== normalizarTextoBasico(registro.usuario_atendente)) return;

    var duracoes = calcularDuracoes(registro);
    if (params.somenteForaSla && duracoes.total !== null && duracoes.total <= config.slaMinutos) return;

    registro._duracoes = duracoes;
    registros.push(registro);
  });

  return { periodos: periodos, registros: registros };
}

function calcularKpisDashboard(registros, config, periodos) {
  var hoje = new Date();
  var ontem = new Date();
  ontem.setDate(hoje.getDate() - 1);
  var semanaInicio = new Date();
  semanaInicio.setDate(hoje.getDate() - 6);
  var resumo = {
    solicitacoesHoje: 0,
    solicitacoesOntem: 0,
    ...
    taxaConclusao: 0,
    taxaCancelamento: 0,
    tempoTotalMedio: null,
    tempoAtendimentoMedio: null,
    tempoFilaMedio: null,
    p95: null,
    p99: null,
    slaPercentual: 0,
    slaMeta: config.slaMinutos
  };
  ...
  var canceladas = 0;
  var temposTotal = [];
  var temposAtendimento = [];
  var temposFila = [];

  registros.forEach(function(r) {
    var criacao = converterParaData(r.timestamp_criacao);
    if (!criacao) return;
    if (criacao.toDateString() === hoje.toDateString()) resumo.solicitacoesHoje++;
    if (criacao.toDateString() === ontem.toDateString()) resumo.solicitacoesOntem++;
    if (criacao >= semanaInicio) resumo.solicitacoesSemana++;
    if (criacao.getMonth() === hoje.getMonth() && criacao.getFullYear() === hoje.getFullYear()) resumo.solicitacoesMes++;

    var statusNorm = normalizarTextoBasico(r.status);
    if (statusNorm === 'aberto' || statusNorm === 'pendente') resumo.abertasAgora++;
    if (statusNorm === 'em_producao' || statusNorm === 'em-producao' || statusNorm === 'em producao') resumo.emProducaoAgora++;
    if (statusNorm === 'entregue' || statusNorm === 'concluido' || statusNorm === 'concluído') concluidas++;
    if (statusNorm === 'cancelado') canceladas++;
    if (statusNorm === 'entregue' && criacao.toDateString() === hoje.toDateString()) resumo.entreguesHoje++;

    if (r._duracoes) {
      if (r._duracoes.total !== null && !isNaN(r._duracoes.total)) temposTotal.push(r._duracoes.total);
      if (r._duracoes.atendimento !== null && !isNaN(r._duracoes.atendimento)) temposAtendimento.push(r._duracoes.atendimento);
      if (r._duracoes.fila !== null && !isNaN(r._duracoes.fila)) temposFila.push(r._duracoes.fila);
      if (r._duracoes.total !== null && r._duracoes.total > config.slaMinutos) resumo.foraSla++;
    }
  });

  resumo.taxaConclusao = total ? Math.round((concluidas / total) * 100) : 0;
  resumo.taxaCancelamento = total ? Math.round((canceladas / total) * 100) : 0;
  resumo.tempoTotalMedio = temposTotal.length ? temposTotal.reduce(function(a, b) { return a + b; }, 0) / temposTotal.length : null;
  resumo.tempoAtendimentoMedio = temposAtendimento.length ? temposAtendimento.reduce(function(a, b) { return a + b; }, 0) / temposAtendimento.length : null;
  resumo.tempoFilaMedio = temposFila.length ? temposFila.reduce(function(a, b) { return a + b; }, 0) / temposFila.length : null;
  resumo.p95 = calcularPercentil(temposTotal, 95);
  resumo.p99 = calcularPercentil(temposTotal, 99);
  resumo.slaPercentual = total ? Math.round(((total - resumo.foraSla) / total) * 100) : 0;
  resumo.canceladasPeriodo = canceladas;

  return resumo;
}

function gerarRankingsDashboard(registros) {
  function ordenar(objeto, chave) {
    return Object.keys(objeto).map(function(k) { return { chave: k, total: objeto[k] }; }).sort(function(a, b) { return b.total - a.total; });
  }
  var armarios = {};
  var solicitantes = {};
  var atendentes = {};
  registros.forEach(function(r) {
    var arm = normalizarIdentificador(r.armario_id) || 'Sem armário';
    armarios[arm] = (armarios[arm] || 0) + 1;
    var sol = normalizarTextoBasico(r.usuario_solicitante) || 'N/D';
    solicitantes[sol] = (solicitantes[sol] || 0) + 1;
    var at = normalizarTextoBasico(r.usuario_atendente) || 'N/D';
    atendentes[at] = (atendentes[at] || 0) + 1;
  });
  return {
    armariosTop: ordenar(armarios, 'armario').map(function(item) { return { armario: item.chave, total: item.total }; }),
    solicitantesTop: ordenar(solicitantes, 'solicitante').map(function(item) { return { nome: item.chave, total: item.total }; }),
    atendentesTop: ordenar(atendentes, 'atendente').map(function(item) { return { nome: item.chave, total: item.total }; })
  };
}

function gerarCsvDashboard(registros) {
  ...
  registros.forEach(function(r) {
    var linha = cabecalhos.map(function(c) {
      var valor = r[c] !== undefined ? r[c] : '';
      if (valor && Object.prototype.toString.call(valor) === '[object Date]') {
...
function getDashboardData(params) {
  try {
    var config = carregarConfiguracoesDashboard();
    var resultado = filtrarLogsDashboard(params || {}, config);
    var registros = resultado.registros;
    var kpis = calcularKpisDashboard(registros, config, resultado.periodos);
    var graficos = {
      volumeDia: agruparPorDia(registros),
      status: agruparStatus(registros),
      tempos: {
        filaMedio: kpis.tempoFilaMedio || 0,
        atendimentoMedio: kpis.tempoAtendimentoMedio || 0,
        totalMedio: kpis.tempoTotalMedio || 0
      },
      heatmap: montarHeatmap(registros)
    };
    var rankings = gerarRankingsDashboard(registros);
    var armariosResumo = calcularResumoArmarios(registros);
    var csv = gerarCsvDashboard(registros);
    return {
      success: true,
      data: {
        kpis: kpis,
        graficos: graficos,
        rankings: rankings,
        armariosResumo: armariosResumo,
        carimboAtualizacao: Utilities.formatDate(new Date(), resultado.periodos.tz, 'dd/MM/yyyy HH:mm'),
        periodo: { inicio: resultado.periodos.inicioTexto, fim: resultado.periodos.fimTexto },
        exportacao: { csv: csv }
      }
    };
  } catch (error) {
...
function salvarSnapshotDashboard(payload) {
  try {
    garantirEstruturaDashboard();
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('SNAPSHOTS');
    var k = payload && payload.kpis ? payload.kpis : {};
    var filtros = payload && payload.filtros ? payload.filtros : {};
    sheet.appendRow([
      new Date(),
      filtros.dataInicio || '',
      filtros.dataFim || '',
      k.solicitacoesSemana || 0,
      k.taxaConclusao || 0,
      k.taxaCancelamento || 0,
      k.taxaConclusao || 0,
      k.taxaCancelamento || 0,
      k.slaPercentual || 0,
      k.slaMeta || '',
